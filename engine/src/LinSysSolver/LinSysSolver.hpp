    ...
    virtual void set_pattern(const std::vector<std::set<int>> &vNeighbor,
        ...
        for (int rowI = 0; rowI < vNeighbor.size(); rowI++) {
            if (fixedVert.find(rowI) == fixedVert.end()) {
                ...
                for (const auto &colI : vNeighbor[rowI]) {
                    if (fixedVert.find(colI) == fixedVert.end()) {
                        if (colI > rowI) {
                            ...
                            // colI > rowI means upper-right, but we are
                            // preparing CSR here in a row-major manner and
                            // CHOLMOD is actually column-major
                            int oldSize_ja_temp = static_cast<int>(ja.size());
                            IJ2aI[rowI * DIM][colI * DIM] = oldSize_ja_temp;
                            IJ2aI[rowI * DIM][colI * DIM + 1] =
                                oldSize_ja_temp + 1;
                            if (DIM == 3) {
                                IJ2aI[rowI * DIM][colI * DIM + 2] =
                                    oldSize_ja_temp + 2;
                            }
                ...
                ja.bottomRows(nnz_rowI * DIM - 1) =
                    ja.block(oldSize_ja + 1, 0, nnz_rowI * DIM - 1, 1);
                ...
                if (DIM == 3) {
                    ...
                    ja.bottomRows(nnz_rowI * DIM - 2) =
                        ja.block(oldSize_ja + 2, 0, nnz_rowI * DIM - 2, 1);
