...
class Scaffold {
  public:
    ...
    Eigen::VectorXi bnd, localVI2Global; // map between airMesh indices to
  ...
  public:
    ...
    Scaffold(const TriMesh &mesh, Eigen::MatrixXd UV_bnds = Eigen::MatrixXd(),
             ...
             const Eigen::VectorXi &p_bnd = Eigen::VectorXi());

    // augment mesh gradient with air mesh gradient with parameter w_scaf
    void augmentGradient(Eigen::VectorXd &gradient,
    ...
    void wholeSearchDir2airMesh(const Eigen::VectorXd &searchDir,
    ...
    void mergeFixedV(const std::set<int> &fixedV_mesh,
