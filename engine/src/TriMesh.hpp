...
enum Primitive { P_SQUARE, P_CYLINDER };
class Scaffold;
...
class TriMesh {
  public:                   // owned data
    ...
    Eigen::MatrixXd V; // duplicated vertex coordinates, the dimension depends
                       // on the search space
    Eigen::MatrixXi
        F; // reordered triangle draw list (0, 1, 2, ...), indices based on V
    Eigen::MatrixXi
        cohE; // cohesive edge pairs with the 4 end vertex indices based on V
  ...
  public: // constructor
    ...
    // initialize from a triangle mesh, V will be constructed from UV_mesh in
    // 2D, V_mesh will be used to initialize restShape
    TriMesh(const Eigen::MatrixXd &V_mesh, const Eigen::MatrixXi &F_mesh,
            const Eigen::MatrixXd &UV_mesh,
            const Eigen::MatrixXi &FUV_mesh = Eigen::MatrixXi(),
            bool separateTri = true, double p_initSeamLen = 0.0,
            double p_areaThres_AM = 0.0);
  ...
  public: // API
    ...
    bool splitEdge(double lambda_t, double EDecThres = 0.0,
                   bool propagate = false, bool splitInterior = false);
    void queryMerge(double lambda, bool propagate, double &EwDec_max,
    ...
    bool checkInversion(bool mute = false, const std::vector<int> &triangles =
                                               std::vector<int>()) const;

    bool save(const std::string &filePath, const Eigen::MatrixXd &V,
              const Eigen::MatrixXi &F, const Eigen::MatrixXd UV,
              const Eigen::MatrixXi &FUV = Eigen::MatrixXi()) const;
    bool save(const Eigen::MatrixXd &V, const Eigen::MatrixXi &F,
              const Eigen::MatrixXd UV,
              const Eigen::MatrixXi &FUV = Eigen::MatrixXi()) const;
    bool saveAsMesh(const std::string &filePath, const Eigen::MatrixXi &F0,
                    bool scaleUV) const;
    bool saveAsMesh(const Eigen::MatrixXi &F0, bool scaleUV) const;
  ...
  public: // helper function
    ...
    bool insideUVRegion(const std::vector<int> &triangles,
                        const Eigen::RowVector2d &pos) const;

    // toBound = false indicate counter-clockwise
    bool isBoundaryVert(int vI, int vI_neighbor, std::vector<int> &tri_toSep,
                        std::pair<int, int> &boundaryEdge,
    ...
    void splitEdgeOnBoundary(const std::pair<int, int> &edge,
                             ...
                             bool allowCutThrough = true);
    void mergeBoundaryEdges(const std::pair<int, int> &edge0,
    ...
    double computeLocalLDec(
        ...
        Eigen::MatrixXd &newVertPos, std::pair<double, double> &energyChanges,
    ...
    // merge
    double
    computeLocalEdDec_merge(const std::vector<int> &path,
